...
class GameFunctionalTest extends AnyFlatSpec with Matchers {
  "A Game" should "be able to create a new canvas, place 4 types of ships, only one per type, then attack" in {
    ...
    // first submarine
    val firstShip = placeShip("B", "2", "B", "2", newCanvas)

    // second submarine not placed, violates location and one ship per type rules
    val secondShip = placeShip("C", "2", "C", "2", firstShip)

    // first destroyer
    val thirdShip = placeShip("D", "4", "E", "4", secondShip)

    // first cruiser
    val fourthShip = placeShip("B", "4", "B", "6", thirdShip)

    // first battleship
    val fifthShip = placeShip("J", "6", "J", "9", fourthShip)

    // second battleship not placed, violates location and one ship per type rules
    val sixthShip = placeShip("I", "6", "I", "9", fifthShip)

    val attackOne = attack(
      coordinateX = "A",
      coordinateY = "1",
      gameState =
        AttackPhase(
